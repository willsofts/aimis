...
$(function(){
	...
	try { startApplication("filterquest"); }catch(ex) { }
	initialApplication();
...
function search(aform) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/filterquest/search",
...
function insert() {
	...
	aform.filterid.value = "";
	let formdata = serializeDataForm(aform);
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/filterquest/add",
		data: formdata.jsondata,
		headers : formdata.headers,
		type: "POST",
		dataType: "html",
...
function save(aform) {
	...
	$("#forumcategories").parent().removeClass("has-error");
	$("#forumcategories_alert").hide();
	...
	validSaveForm(function() {
		...
		if(!validateforumcategories()) {
			$("#forumcategories").parent().addClass("has-error");
			$("#forumcategories_alert").show();
			return;
		}
		...
		confirmSave(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/filterquest/insert",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function update(aform) {
	...
	validSaveForm(function() {
		...
		confirmUpdate(function() {
			...
			startWaiting();
			jQuery.ajax({
				url: API_URL+"/api/filterquest/update",
				data: formdata.jsondata,
				headers : formdata.headers,
				type: "POST",
				dataType: "html",
...
function submitRetrieve(src,filterid) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/filterquest/retrieval",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function deleteRecord(fsParams) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/filterquest/remove",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "html",
...
function validateforumcategories() {
	let isz = $("input.forum-categories").filter(":checked").length;
	console.log("forum categories: checked",isz);
	if(isz >= 2) return true;
	return false;
}
function sendToParent(action) {
	try { 
		window.parent.talkAboutIt({action: action, filterid: $("#filterid").val(), filtername: $("#filtername").val()});
	} catch(ex) { }
}
function openForum(forumid) {
	...
	startWaiting();
	jQuery.ajax({
		url: API_URL+"/api/filterquest/forum",
		data: formdata.jsondata,
		headers: formdata.headers,
		type: "POST",
		dataType: "json",
...
function saveForum(aform) {
	if($.trim($("#classifyprompt").val())=="") {
		$("#classifyprompt").parent().addClass("has-error");
		$("#classifyprompt_alert").show();
		$("#classifyprompt").focus();
		return;
	}
	if(!aform) aform = fsentryformforum;
	confirmUpdate(function() {
		let formdata = serializeDataForm(aform);
		startWaiting();
		jQuery.ajax({
			url: API_URL+"/api/filterquest/forumupdate",
			data: formdata.jsondata,
			headers : formdata.headers,
			type: "POST",
			dataType: "html",
			...
			success: function(data,status,transport){ 
				stopWaiting();
				successbox(function() { 
					$("#fsmodaldialog_layer_forum").modal("hide");
				});
			}
		});
	});
