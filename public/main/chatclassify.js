...
$(function() {
	...
	$("#addforumlinker").click(function() { 
		open_program("filterquest","",null,"/gui/filterquest/entry",true);
	});
...
function sendQuery(quest) {
	...
	if(!cat || cat=="") cat = "QUESTCLASSIFY";
	let authtoken = getAccessorToken();
	jQuery.ajax({
		url: API_URL+"/api/chatter/quest",
		data: {category: cat, mime: "CLASSIFY", query: quest},
...
function loadCategories(forumid) {
	...
	let authtoken = getAccessorToken();
	jQuery.ajax({
		url: API_URL+"/api/filterquest/list",
		type: "POST",
